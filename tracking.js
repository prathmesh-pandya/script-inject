 ...
 * Send data to your backend for tracking and analysis.
 */
...
(function () {
    ...
    window.visitorTracker = {
        ...
        getBrowserInfo: function () {
            ...
            // Chrome
            if (/Chrome/.test(userAgent) && !/Chromium|Edge|Edg|OPR|Opera|brave/i.test(userAgent)) {
                browserName = 'Chrome';
            ...
            // Safari
            else if (/Safari/.test(userAgent) && !/Chrome|Chromium|Edge|Edg|OPR|Opera/i.test(userAgent)) {
                browserName = 'Safari';
        ...
        // Log visitor data to console (instead of sending to server)
        sendToServer: function (visitorData) {
            // Convert the data to the format your backend expects
            const formattedString = this.formatVisitorString(visitorData);

            // Create the payload that would go to the server
            const payload = {
                visitorIdentifier: formattedString,
                fullData: visitorData,
                url: window.location.href,
                timestamp: new Date().toISOString()
            };

            // Log the data to console instead of sending to server
            console.log('%c Visitor Tracking Data ', 'background: #4834d4; color: white; padding: 2px 6px; border-radius: 2px; font-weight: bold;', payload);
            console.log('Visitor identifier string:', formattedString);
            console.log('This string would be sent to backend to combine with IP address');
        }
    };
